...
const ChartTooltipContent = React.forwardRef<
  HTMLDivElement,
  React.ComponentProps<"div"> &
    RechartsTooltipContentProps<TooltipValueType, string | number> & {
      hideLabel?: boolean;
...
>(
  (
    ...
    const tooltipPayload = (payload ?? []) as ReadonlyArray<TooltipPayloadEntry<TooltipValueType, string | number>>;
      ...
      >
        ...
        <div className="grid gap-1.5">
          {tooltipPayload.map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const payloadEntry = item.payload as Record<string, unknown> | undefined;
            const indicatorColor = color || (typeof payloadEntry?.fill === "string" ? payloadEntry.fill : undefined) || item.color;
